#!/usr/bin/env python3
"""
Robust Data Extractor for Atlanta Biotech Network

This script uses a more sophisticated approach to extract all nodes and links
from the complex JavaScript data structure, handling nested objects properly.
"""
...
def extract_robust_data(js_file_path):
    """Extract network data using robust parsing approach."""
    print("Robust network data extraction...")
    ...
    # Extract nodes using a more sophisticated approach
    nodes = extract_nodes_robust(object_content)
    
    # Extract links
    links = extract_links_robust(object_content)
    
    # Extract node colors
    node_colors = extract_node_colors_robust(object_content)
...
def extract_nodes_robust(content):
    """Extract nodes using robust parsing that handles nested structures."""
    print("Extracting nodes...")
    
    # Find the nodes array start
    ...
    # Use a more sophisticated approach to find complete node objects
    # Look for { id: "..." and find the matching closing brace
    pos = 0
    while pos < len(nodes_content):
        # Find next node start
        node_start = nodes_content.find('{ id:', pos)
        if node_start == -1:
            break
        ...
        node_obj = parse_node_robust(node_str)
        if node_obj:
...
def extract_links_robust(content):
    """Extract links using robust parsing."""
    print("Extracting links...")
    
    # Find the links array start
    ...
    # Find all link objects by looking for the pattern: { source: "...", target: "..." }
    link_objects = []
    
    # Use a more sophisticated approach to find complete link objects
    pos = 0
    while pos < len(links_content):
        # Find next link start
        link_start = links_content.find('{ source:', pos)
        if link_start == -1:
            break
        ...
        link_obj = parse_link_robust(link_str)
        if link_obj:
...
def parse_node_robust(node_str):
    """Parse a node object with robust property extraction."""
    try:
        node_obj = {}
...
def parse_link_robust(link_str):
    """Parse a link object with robust property extraction."""
    try:
        link_obj = {}
...
def extract_node_colors_robust(content):
    """Extract node colors using robust parsing."""
...
def main():
    """Extract robust network data and save to JSON."""
    js_file = 'src/atlanta_biotech_data.js'
    json_file = 'biotech_network_data.json'
    
    print("Robust Atlanta Biotech Network Data Extractor")
    print("=" * 50)
    
    try:
        data = extract_robust_data(js_file)
