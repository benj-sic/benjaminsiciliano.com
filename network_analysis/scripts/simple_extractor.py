#!/usr/bin/env python3
"""
Simple Network Data Extractor

This script uses a simple approach to extract nodes and links by manually
parsing the JavaScript structure using brace counting.
"""
...
def extract_simple_data(js_file_path):
    """Extract network data using simple brace counting approach."""
    print("Simple network data extraction...")
    ...
    # Extract nodes using simple approach
    nodes = extract_nodes_simple(object_content)
    
    # Extract links using simple approach
    links = extract_links_simple(object_content)
    
    # Extract node colors
    node_colors = extract_node_colors_simple(object_content)
...
def extract_nodes_simple(content):
    """Extract nodes using simple brace counting."""
    ...
    while current_pos < len(nodes_content):
        ...
        node_obj = parse_node_simple(node_str)
        if node_obj:
...
def extract_links_simple(content):
    """Extract links using simple brace counting."""
    ...
    while current_pos < len(links_content):
        ...
        link_obj = parse_link_simple(link_str)
        if link_obj:
...
def parse_node_simple(node_str):
    """Parse a node object using simple regex."""
    try:
        node_obj = {}
...
def parse_link_simple(link_str):
    """Parse a link object using simple regex."""
    try:
        link_obj = {}
...
def extract_node_colors_simple(content):
    """Extract node colors using simple approach."""
...
def main():
    ...
    try:
        data = extract_simple_data(js_file)
