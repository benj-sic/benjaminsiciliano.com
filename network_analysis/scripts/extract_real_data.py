#!/usr/bin/env python3
"""
Real Data Extractor for Atlanta Biotech Network

This script extracts the actual network data from your JavaScript file
by using a more robust approach that handles the complex nested structure.
"""
...
def extract_real_data(js_file_path):
    """Extract real network data from JavaScript file."""
    print("Extracting real Atlanta biotech network data...")
    ...
    nodes = extract_nodes_real(object_content)
    
    # Extract links
    links = extract_links_real(object_content)
    
    # Extract node colors
    node_colors = extract_node_colors_real(object_content)
...
def extract_nodes_real(content):
    """Extract nodes using a more sophisticated approach."""
    ...
    while pos < len(nodes_content):
        ...
        node_obj = parse_node_real(node_str)
        if node_obj:
...
def extract_links_real(content):
    """Extract links using a more sophisticated approach."""
    ...
    while pos < len(links_content):
        ...
        link_obj = parse_link_real(link_str)
        if link_obj:
...
def extract_node_colors_real(content):
    """Extract node colors using a more sophisticated approach."""
...
def main():
    """Extract real network data and save to JSON."""
    js_file = 'src/atlanta_biotech_data.js'
    json_file = 'biotech_network_data.json'
    
    print("Real Atlanta Biotech Network Data Extractor")
    print("=" * 50)
    
    try:
        data = extract_real_data(js_file)
